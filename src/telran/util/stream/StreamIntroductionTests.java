...
//		return Arrays.stream(ar).mapToLong(a -> Arrays.stream(a).asLongStream().sum()).sum();
	...
	void evenOddTestRandom() {
		...
		mapEvenOdd = new Random().ints(10, 1, 100).boxed()
	...
	private String getOccurrences(String str) {
		return Arrays.stream(str.split("[^A-Za-z]+"))//stream of words
				.collect(Collectors.groupingBy(s -> s, TreeMap::new, Collectors.counting()))//grouping word: occurences
				.entrySet().stream().sorted((e1, e2) -> Long.compare(e2.getValue(), e1.getValue()))//sorted by occurenses in descending
